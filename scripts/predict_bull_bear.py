...
def predict_input(
    ...
    if not os.path.exists(itos_file_path):
        print("Could not find " + itos_file_path)
        exit(-1)

    # Check the classifier file exists
    if not os.path.exists(trained_classifier_file_path):
        print("Could not find " + trained_classifier_file_path)
        exit(-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tcf', '--trained_classifier_file_path', help='Path for the data file. If not specified, we\'ll read the data from stdin', type=str, required=True)
    parser.add_argument('-bs', '--batch_size', help='Number of records per read.', type=int, default=5)
    parser.add_argument('-idf', '--input_data_file_path', help='Path for the data file. If not specified, we\'ll read the data from stdin', type=str, required=False)
